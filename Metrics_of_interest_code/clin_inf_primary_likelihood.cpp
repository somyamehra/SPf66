 ...
 *    FORI: vector of length N_OBS+N_WINDOWS for FORI
 *    NO_PRIM_WINDOW: window for which we assume the absence of primary infection
 *    K: shape parameter for Gamma distribution governing lambda (FORI scaling factor)
 *    THETA: scale parameter for Gamma distribution governing lambda (FORI scaling factor) */
...
// [[Rcpp::export]]
vec log_joint_PGF_noprim_window 
  (mat Z, int N_OBS, int N_AGE, double TIME_STEP, double ETA, double NU, 
   double PCLIN, double PREL, vec FORI, uvec NO_PRIM_WINDOWS) {
  ...
  // set column NO_PRIM_WINDOW of Z to zero -> enforce no primary infection
  mat Z_mod = Z;
  Z_mod.cols(NO_PRIM_WINDOWS-1) *= 0;
  ...
  mat z_function_1 = (1 - PCLIN + PCLIN*Z_mod)/(ETA*(1 + NU*PCLIN*PREL - NU*PCLIN*PREL*Z));
  mat coeff_study_1 = z_function_1.each_row() % FORI.subvec(N_AGE, N_OBS+N_AGE-1).t();

  mat z_function_2 = (PCLIN - PCLIN*Z_mod)/(ETA*(1 + NU*(1-PREL) + NU*PCLIN*PREL - NU*PCLIN*PREL*Z));
...
// [[Rcpp::export]]
double inc_exc_likelihood_noprim_window 
  (mat S_plus, mat S_minus, int N_AGE, double PCLIN, double PREL, int N_OBS, 
   double TIME_STEP, double ETA, double NU, vec FORI_full,
   uvec NO_PRIM_WINDOWS) {
  
  vec FORI = FORI_full.tail(N_AGE+N_OBS);
  
  if (S_minus.n_rows==0) {
    return(exp(log_joint_PGF_noprim_window(S_plus, N_OBS, N_AGE, TIME_STEP, ETA, 
                                          NU, PCLIN, PREL, FORI, NO_PRIM_WINDOWS).front()));
  }
  
  vec f_plus = log_joint_PGF_noprim_window(S_plus, N_OBS, N_AGE, TIME_STEP, ETA, NU, 
                                           PCLIN, PREL, FORI, NO_PRIM_WINDOWS);
  
  vec f_minus = log_joint_PGF_noprim_window(S_minus, N_OBS, N_AGE, TIME_STEP, ETA, NU, 
                                            PCLIN, PREL, FORI, NO_PRIM_WINDOWS);
